...
自动脚本源码见 [`tools/agent-research-weekly`](tools/agent-research-weekly/README.md)，代码版本独立使用 `agent-weekly-vVERSION` Release 标签。定时周报在新推送前增加双角色逐轮追问、局部返修和复核；执行记录保留在运行主机，不放入博客仓库。源码包使用默认关闭自动发布的示例配置，安装方法、预算和限制见工具说明。
