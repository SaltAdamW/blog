...
本期选读营销分析、推理评测与模型研发，补入 DSH 预发布更新和 Paper2Agent 期刊版；简讯关注并行工具调用的故障恢复，以及强化学习中的工具选择捷径。
...
## Paper2Agent：论文里的代码，怎样变成可调用的研究工具

原文：[Reimagining Research Papers as Interactive and Reliable AI Agents](https://www.nature.com/articles/s41586-026-11044-y) · Nature 期刊版发布于 2026-09-16；[预印本](https://arxiv.org/abs/2509.06917)首次提交于 2025-09-08

论文附了代码，不代表另一位研究者马上能用。环境依赖、数据入口、教程步骤和函数参数，往往还需要人工重新拼起来。Paper2Agent 把这段准备工作拆成可执行流程：配置环境、运行教程、提取可复用工具、测试，再通过 MCP 提供调用入口。测试未通过的函数不进入最终工具集。

期刊版报告在 100 篇计算生物学论文中，成功为 74 篇构建了可执行 Agent。剩余失败也有明确原因，包括代码、数据或依赖不可获得。这说明把论文转成工具仍受原始研究制品约束；语言模型不能补出本来就没有公开的实验条件。文中另有 26 篇资料型论文的资源层评测，不能与这 100 篇执行工具的成功比例混在一起。

即使教程能复跑，科学判断也没有自动完成。研究者仍需决定要问什么、结果是否符合当前实验条件，以及后续应该验证哪种假说。Paper2Agent 解决的是从文稿和代码到可测试调用接口的距离，不是把可执行性当作科学结论正确性的证明。本期收录的是期刊版更新，不是方法首次亮相。
